...
import {
  getManualQrConfig,
  isManualQrReady,
  normalizeManualQrProvider,
} from '@/lib/server/manual-qr';
...
async function fetchActiveProduct(productId: number): Promise<ProductRow | null> {
  const rows = (await sql`
    SELECT p.id, p.name, p.slug, p.price, p.sale_price, p.sale_end_at,
           p.image_url, p.colors, c.name AS category_name, c.slug AS category_slug
    FROM products p
...
export async function POST(req: NextRequest) {
  ...
  try {
    body = (await req.json()) as CreateQrBody;
  } catch {
    return badRequest('Payload không hợp lệ');
  }

  const provider = normalizeManualQrProvider(body.provider);
  if (!provider) return badRequest('Phương thức QR không hợp lệ');

  const qrConfig = getManualQrConfig(provider);
  if (!isManualQrReady(qrConfig)) {
    return jsonError(`Chưa cấu hình ảnh ${qrConfig.label} trong .env`, 503);
  }
  ...
  const paymentUrl = `/thanh-toan/qr?order=${encodeURIComponent(orderCode)}`;
  ...
  try {
    await sql`
      INSERT INTO orders (
        ...
        customer_note, payment_provider, payment_method, payment_url, status
      )
      VALUES (
        ${orderCode}, ${product.id}, ${JSON.stringify(productSnapshot)}::jsonb,
        ${selectedColor}, ${quantity}, ${amount}, 'VND', ${customerName},
        ${customerPhone}, ${customerEmail}, ${customerNote}, ${provider},
        ${qrConfig.paymentMethod}, ${paymentUrl}, 'awaiting_transfer'
      )
    `;
    ...
    const msg = err instanceof Error ? err.message : 'Không thể tạo đơn thanh toán QR';
